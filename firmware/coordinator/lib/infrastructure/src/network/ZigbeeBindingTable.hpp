...
public:
    void onDeviceAnnounced(uint16_t short_addr, uint64_t ieee) noexcept;
    void onDeviceLeft     (uint16_t short_addr)                noexcept;
    [[nodiscard]] std::optional<gh::domain::NodeId>
        resolve(uint16_t short_addr) const noexcept;
